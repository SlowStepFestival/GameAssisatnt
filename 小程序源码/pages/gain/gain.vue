<template>
	<view>
		<view class="title">成就列表</view>
		
		
		<view class="uni-list">
			<view class="uni-list-cell" hover-class="uni-list-cell-hover" v-for="(item,index) in list" :key="index">
				<view class="uni-media-list">
					<image class="uni-media-list-logo" :src="item.imageUrl"></image>
					<view class="uni-media-list-body">
						<view class="uni-media-list-text-top">{{item.achievementName}}</view>
						<view class="uni-media-list-text-bottom uni-ellipsis">{{item.profile}}</view>
					</view>
				</view>
			</view>
		</view>
	...
	export default {
		onLoad(e) {
			uni.request({
				...
				data: {},
				success: res => {
					this.list=res.data.achievementList;
